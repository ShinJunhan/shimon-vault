# terraform/app_ec2_green.tf
# count = 0 at rest. Set deploy_green = true during a deployment.
resource "aws_instance" "app_green" {
  ...
  # IMPORTANT: without this, changing user_data (e.g. rotating the Tailscale
  # auth key, editing the boot script) only updates the stored value in
  # Terraform state — it does NOT reboot or re-run the script on the existing
  # instance. A previously-failed instance stays broken forever even after
  # the underlying script is fixed. This forces a real replacement so the
  # corrected script actually runs on a fresh boot.
...
output "app_green_private_ip" {
  value = length(aws_instance.app_green) > 0 ? aws_instance.app_green[0].private_ip : "not deployed"
}
